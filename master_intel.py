...
def clean(text):
    if not text: return ""
    # 🌟 强力去杂质：抹除 Markdown 符号和 AI 话痨标记
    text = str(text).replace('*', '').replace('#', '').replace('「', '').replace('」', '').replace('解释：', '').replace('例句：', '')
...
def get_ai(prompt_type, ctx=""):
    prompts = {
        "word": "从 TUYU 歌词或日语生活中选一个词，标注假名。只回词，禁止 Markdown。",
        "desc": f"解释词语 '{ctx}'。若是歌词请注出处。禁止加粗，禁止 Markdown。",
        "japan": "总结 2026 年 3 月日本科技简报。三句，禁止陈旧信息。",
        "lyric": f"提供歌曲《{ctx}》的日文歌词和中文翻译。格式: 歌词|翻译",
        "news": "搜集三条 2026 年 3 月全球网安或金融新闻。格式: 标题|URL。必须真实可访问。",
        "cve": f"审计 CVE。格式: 评分|描述|建议。禁止废话。内容:{ctx}"
    }
    try:
...
def git_sync():
    ...
    try:
        ...
        msg = f"Stable Recon: {datetime.now().strftime('%H:%M')}"
...
# 🌟 新闻抓取 + 自动验证
news_html = ""
news_list = get_ai("news").split('\n')
valid_count = 0
for line in news_list:
    if '|' in line and valid_count < 3:
        title, url = line.split('|', 1)
        url = url.strip()
        # 验证链接有效性，无效则跳过
        if check_link(url):
            news_html += f'<div style="margin-bottom:12px;">• {clean(title)} <a href="{url}" target="_blank" style="color:var(--b);font-size:10px;text-decoration:none;text-shadow:0 0 5px var(--b);">[LINK]</a></div>'
            valid_count += 1
...
try:
    ...
    for repo in r.json().get("items", [])[:8]:
        ...
        <div class="cve-card">
            ...
            <div class="cve-body" style="line-height:1.8; margin-top:5px; opacity:0.9;">{clean(intel[1])}</div>
            <div class="cve-hint" style="font-size:10px; margin-top:4px; opacity:0.6;">📡 {clean(repo["owner"]["login"])} | 💡 {clean(intel[2])}</div>
        </div>'''
except: cve_html = "<p>Data Offline</p>"

# --- HTML 模板 (死磕 TUYU 视觉层级) ---
...
body {{ 
    ...
    background-image: linear-gradient(rgba(0,0,0,0.4), rgba(0,0,0,0.7)), url('{mv_bg}'); 
    background-size:cover; background-position:center; background-attachment:fixed; height:100vh; overflow:hidden; 
}}
.glass {{ 
    background: var(--bg); backdrop-filter: blur(25px); -webkit-backdrop-filter: blur(25px); 
    border: 1px solid rgba(255, 255, 255, 0.1); border-top: 1px solid rgba(255, 255, 255, 0.25); 
    box-shadow: 0 8px 32px 0 rgba(0, 0, 0, 0.6); border-radius: 12px; padding: 18px; 
    display:flex; flex-direction:column; overflow:hidden;
}}
.header {{ height:100px; display:flex; align-items:center; gap:22px; margin-bottom:15px; border-bottom:1px solid rgba(255, 0, 127, 0.4); }}
...
.cve-link {{ color:var(--p); font-weight:bold; text-decoration:none; font-size:14.5px; text-shadow: 0 0 5px rgba(255,0,127,0.3); }}
.neon-blue {{ color: var(--b); text-shadow: 0 0 8px rgba(0, 212, 255, 0.6); }}
.cve-tag {{ border: 1px solid var(--b); color: var(--b); padding: 0 6px; border-radius: 3px; font-size: 10px; font-weight: bold; box-shadow: 0 0 5px rgba(0,212,255,0.4) inset; }}
...
git_sync()
print("情报站已加固：Link 验证已开启，UI 干扰项已清洗。")
