...
def get_ai(prompt_type, ctx=""):
    prompts = {
        ...
        "cve": f"作为网安专家，审计该CVE仓库。格式严格: 评分|描述|建议。内容:{ctx}"
    }
    try:
...
.header img {{ 
    width:75px; height:75px; border-radius:8px; border:1.5px solid var(--p); 
...
<div class="container">
    <div style="display:flex;flex-direction:column;gap:15px;">
        <div class="glass" style="flex:1.2;display:flex;flex-direction:column;overflow:hidden;">
        ...
        <div class="glass" style="flex:1;border-left:4px solid var(--b);">
            <span class="label">MARKET_RECON</span>
            <div class="scroll" style="font-size:11.5px;line-height:1.6;color:#99f1ff;text-shadow: 1px 1px 2px rgba(0,0,0,0.8);">{news}</div>
        </div>
...
git_sync()
send_bark("TUYU_CyberIntel", f"UI 视觉已完美还原。当前背景: 《{chosen_song}》")
print("自动化审计完成，UI 结构已锁定。")
