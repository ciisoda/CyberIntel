...
GITHUB_HEADERS = {
    ...
    "User-Agent": "marble-soda-intel-v30",
...
def get_ai_intel(text, intel_type="cve"):
    ...
    prompts = {
        "cve": f"分析漏洞返回：评分|研判总结(60字内)|风险建议。禁止Markdown：{text}",
        "japan": f"以3个短句总结日本IT/留学动态。禁止Markdown：{text}",
        "word": "选网安或音游日语词，回 单词+假名。",
        "desc": f"解释 '{text}' 并给出一个短例句，必须使用换行符分隔。禁止Markdown。",
...
def sync_security():
    ...
    except Exception as e: log(f"❌ 漏洞抓取环节崩溃: {e}")
...
def run():
    ...
    for key in TUYU_ASSETS:
        if key in song_name_raw:
            mv_bg, album_cover = TUYU_ASSETS[key]["bg"], TUYU_ASSETS[key]["cover"]
            break
    ...
    <style>
        body {{ 
            ...
            /* 🚩 优化：降低遮罩浓度，背景更透亮 */
            background: linear-gradient(rgba(5,5,10,0.6), rgba(5,5,10,0.6)), url('{mv_bg}') center/cover fixed no-repeat;
        }}
        ...
        .section-box {{ background: rgba(10, 10, 20, 0.78); backdrop-filter: blur(12px); padding: 25px; border-radius: 15px; border: 1px solid rgba(255,255,255,0.08); height: fit-content; }}
        /* 🚩 优化：左侧文字间距与滚动 */
        .jp-word {{ font-size: 24px; color: #00d4ff; font-weight: bold; margin-bottom: 12px; }}
        ...
        .japan-card {{ margin-top: 15px; padding: 12px; background: rgba(255, 0, 127, 0.08); border-left: 3px solid #ff007f; border-radius: 4px; font-size: 13px; }}
    </style></head><body>
        <div class="lyric-box">
            <img src="{album_cover}" class="album-art">
            <div style="flex:1;">
                <span style="color:#ff007f; font-size:11px; letter-spacing:2px; font-weight:bold;">♫ TUYU_SYSTEM // V30.0</span>
        ...
        <div class="grid">
            ...
            <div class="section-box">
                <h3>🔥 Heat Vulnerability Intelligence</h3>
                {sec_html}
            </div>
        </div>
    </body></html>
